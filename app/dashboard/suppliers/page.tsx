    ...
    <div>
          ...
          <tbody>
            {suppliers.map((s) => (
              <tr key={s.id} className="border-b border-neutral-100 last:border-0">
                ...
                <td className="px-4 py-3 text-neutral-600">{s.contact_person || '—'}</td>
                <td className="px-4 py-3 text-neutral-600">{s.phone || '—'}</td>
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-neutral-100 text-neutral-500'}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-3">
                    ...
                    <button
                      ...
                      className={s.is_active ? 'text-red-500 hover:text-red-700' : 'text-emerald-600 hover:text-emerald-700'}
            ...
            {suppliers.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-neutral-400">
                  <Truck size={28} className="mx-auto mb-2 text-neutral-300" />
                  No suppliers yet.
                </td>
              </tr>
            )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={handleSave} className="w-full max-w-sm space-y-3 rounded-xl bg-white p-6">
            <h3 className="text-lg font-semibold text-neutral-900">{editing ? 'Edit Supplier' : 'Add Supplier'}</h3>
