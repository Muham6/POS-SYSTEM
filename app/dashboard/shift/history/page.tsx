...
export default async function ShiftHistoryPage() {
  ...
  return (
    <div>
      <Link href="/dashboard/shift" className="text-sm text-neutral-500 hover:underline">
        ← My Shift
      </Link>
      <h1 className="mt-2 mb-6 text-2xl font-semibold text-neutral-900">Shift History</h1>
          ...
          <tbody>
            {shifts.map((s) => (
              <tr key={s.id} className="border-b border-neutral-100 last:border-0">
                <td className="px-4 py-3 font-medium text-neutral-900">{s.profiles?.full_name || '—'}</td>
                ...
                <td className={`px-4 py-3 text-right font-medium ${
                  s.variance == null ? 'text-neutral-300' : Math.abs(s.variance) < 0.01 ? 'text-emerald-600' : 'text-red-600'
                }`}>
                  {s.variance != null ? `${s.variance > 0 ? '+' : ''}₦${Number(s.variance).toLocaleString()}` : '—'}
                </td>
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                    s.status === 'open' ? 'bg-blue-50 text-blue-700' : 'bg-neutral-100 text-neutral-500'
                  }`}>
                    {s.status}
                  </span>
                </td>
            ...
            {shifts.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-12 text-center text-neutral-400">
                  <History size={28} className="mx-auto mb-2 text-neutral-300" />
                  No shifts yet.
                </td>
              </tr>
            )}
